...
void Pwm::SetDutyCycle(uint32_t dutyCycle) {
    configASSERT(dutyCycle <= 100);

    int minOutput = 0;
    int maxOutput = (1 << m_pwmTimerConfig.duty_resolution) - 1;
    BaoRescale rescale(0, 100, minOutput, maxOutput);
    

    m_pwmChannelConfig.duty = rescale(dutyCycle);
    esp_err_t res = ledc_set_duty(m_pwmTimerConfig.speed_mode, m_pwmChannelConfig.channel, dutyCycle);


    if (res != ESP_OK) {
        BAO_LOG_ERROR("ledc_set_duty failed: %d", res);
    }
...
void Pwm::configPwm() {
    ...
    BAO_LOG_WARNING("corrected freq %ld", correctedFrequency);

    esp_err_t res = ledc_timer_config(&m_pwmTimerConfig);
    if (res != ESP_OK) {
        BAO_LOG_ERROR("Error config pwm timer %d", res);
        return;
    }

    res = ledc_channel_config(&m_pwmChannelConfig);
    if (res != ESP_OK) {
        BAO_LOG_ERROR("Error config pwm channel %d", res);
    }
}
